            ...
            for lr, depth, width, dense, head in itertools.product([.001, .01], [1, 2, 4, 8], [16, 32, 64, 128], dims, heads):
                for seed in range(1, 11):
                    yield dict(id=index, dataset=ds, model=model, lr=lr, depth=depth,
                               width=width, dense=dense, heads=head, seed=seed, epochs=500)
                    index += 1
...
def execute(lib, t, args, stamp):
    ...
    try:
        ...
        # A completed task is fully represented by result.json. Its training
        # checkpoint is only for interruption recovery and would otherwise
        # make the 48,640-task search consume impractical disk space.
...
def main():
    ...
    if args.mode == 'manifest':
        atomic_json(ROOT / 'manifest.json', {'fingerprint': stamp, 'sources': hashes,
            'epochs': 500, 'seeds': list(range(1,11)), 'cells': CELLS, 'features': FEATURES,
            'selection': 'minimum mean configuration RMSE across all ten seeds; no incomplete groups',
            'tasks': list(tasks())})
        print('MANIFEST', 48640)
        ...
        count = 0
        for t in tasks():
            if t['id'] % args.shard_count != args.shard_index or t['model'] not in args.models.split(','):
                continue
